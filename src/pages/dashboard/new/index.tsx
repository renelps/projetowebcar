...
export function New() {
  ...
  function handleUpload(image: File) {
    ...
    const timestamp = new Date().getTime(); // Obtém o timestamp
    const uidImage = `${uuidV4()}-${timestamp}`; // Garante nome único
      ...
      <div className="w-full bg-white p-3 rounded-md flex flex-col sm:flex-row items-center gap-2">
        ...
        {carImages.map((item) => (
          <div key={item.name} className="w-full h-32 flex items-center justify-center relativa">
            ...
            <img src={item.previewUrl} alt={"foto do carro"} className="w-full rounded-md h-32 object-cover" />
          </div>
        ))}
      </div>
      ...
      <div className="w-full bg-white p-3 rounded-md flex flex-col sm-flex-row gap-2 mt-2">
        <form className="w-full" onSubmit={handleSubmit(onSubmit)}>
          <div className="mb-2">
            ...
            <Input placeholder="Ex: SW4 2.8..." type="text" register={register} name="name" error={errors.name?.message} />
          </div>

          <div className="mb-2">
            <p className="mb-2 font-medium">Modelo do carro</p>
            <Input placeholder="Ex: 2.0 diesel automático..." type="text" register={register} name="model" error={errors.model?.message} />
          </div>

          <div className="flex flex-row gap-3 items-center w-full">
            <div className="w-full">
              <p className="mb-2 font-medium">Ano do carro</p>
              <Input placeholder="Ex: 2025/2025..." type="text" register={register} name="year" error={errors.year?.message} />
            </div>

            <div className="w-full">
              <p className="mb-2 font-medium">Km do carro</p>
              <Input placeholder="Ex: 188.000km" type="text" register={register} name="km" error={errors.km?.message} />
            </div>
          </div>

          <div className="w-full mb-2">
            <p className="mb-2 font-medium">Preço do carro</p>
            <Input placeholder="Ex: R$222.000..." type="text" register={register} name="price" error={errors.price?.message} />
          </div>

          <div className="w-full mb-2">
            <p className="mb-2 font-medium">Cidade onde o carro se localiza</p>
            <Input placeholder="Ex: Pesqueira PE..." type="text" register={register} name="city" error={errors.city?.message} />
          </div>

          <div className="w-1/2 mb-2">
            <p className="mb-2 font-medium">Número do whatsapp</p>
            <Input placeholder="Ex: (**) **** ****..." type="text" register={register} name="whatsapp" error={errors.whatsapp?.message} />
          </div>
          ...
          <button type="submit" className="flex items-center justify-center w-full bg-zinc-900 text-white py-1 rounded-md cursor-pointer">
            Cadastrar
          </button>
        </form>
      </div>
